...
function AddNewTheoryModal({
  ...
  const handleCreateTheoryClick = async () => {
    await theoryApi
      .create(fileInfo)
      .then((res) => {
        notification.success({
          message: NOTIFICATIONS_STATUSES.SUCCESS_TITLE,
          description: 'Теоретический материал успешно добавлен',
        })
        setIsOpen(false)
      })
      .catch((err) =>
  ...
  return (
    <Modal
      ...
      footer={
        <Row justify={'end'}>
          <Button
            onClick={() => handleCreateTheoryClick()}
      ...
      open={isOpen}
      title='Добавление нового теоретического материала'
    >
...
export default AddNewTheoryModal
